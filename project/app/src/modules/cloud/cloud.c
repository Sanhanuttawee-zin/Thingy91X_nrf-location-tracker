...
static void shadow_get(bool delta_only)
{
	...
	/* Skip reporting back to shadow to keep desired state active for repeated polling */
	/* This allows the device to re-read the same LED config every poll cycle */
	LOG_DBG("Shadow data processed, not clearing delta to allow repeated reads");
}
